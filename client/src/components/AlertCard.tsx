...
const STYLES: Record<Severity, { bar: string; text: string; bg: string; label: string }> = {
  high: { bar: "bg-risk-high", text: "text-risk-high", bg: "bg-red-50", label: "Severidad alta" },
  med:  { bar: "bg-risk-med",  text: "text-risk-med",  bg: "bg-amber-50", label: "Severidad media" },
  low:  { bar: "bg-risk-low",  text: "text-risk-low",  bg: "bg-green-50", label: "Severidad baja" },
};
      ...
      <div className="pl-4 pr-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            ...
            <div className="text-[14px] font-semibold mt-0.5 text-deloitte-ink">{title}</div>
            <p className="text-[12px] text-deloitte-slate mt-1 leading-snug">{description}</p>
            {action && (
        ...
        <div className="mt-3 pt-2 border-t border-deloitte-line/60 flex items-center justify-between gap-2">
          {feedback ? (
            ...
            <div className="text-[10px] text-deloitte-mute uppercase tracking-wider">Acción</div>
          )}
          <button
            onClick={onGenerar}
            disabled={!!feedback}
            className="px-3 py-1 text-[11px] font-semibold rounded border border-deloitte-ink bg-deloitte-ink text-white hover:bg-deloitte-slate disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
