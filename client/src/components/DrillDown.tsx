      ...
      >
        ...
        <div className="px-6 py-4 border-b border-deloitte-line bg-deloitte-paper/40 flex items-start justify-between gap-4">
          <div>
            ...
            {subtitle && <div className="text-[12px] text-deloitte-slate mt-0.5">{subtitle}</div>}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1 space-y-5">
          ...
          {metrica && (
            <div className="flex items-center gap-4 p-4 bg-deloitte-paper rounded border border-deloitte-line">
              ...
              <div className="text-[12px] text-deloitte-slate">{metrica.label}</div>
            </div>
          )}

          {/* Descripción */}
          {description && (
            <div className="text-[13px] text-deloitte-slate leading-relaxed">{description}</div>
          )}
          ...
          {rows && rows.length > 0 && (
            <div className="border border-deloitte-line rounded overflow-hidden">
              {rows.map((r, i) => (
                <div key={i} className={`flex items-start justify-between gap-4 px-4 py-2.5 text-[12.5px] ${i % 2 === 0 ? "bg-white" : "bg-deloitte-paper/40"}`}>
                  <div className="text-deloitte-slate">
                    <div className="font-semibold text-deloitte-ink">{r.label}</div>
                    {r.sub && <div className="text-[11px] text-deloitte-mute mt-0.5">{r.sub}</div>}
                  </div>
                ...
                <thead className="bg-deloitte-paper">
                  <tr className="text-left">
                    {table.headers.map((h, i) => (
                      <th key={i} className="px-3 py-2 text-[10px] uppercase tracking-wider font-bold text-deloitte-mute">{h}</th>
                    ))}
                  </tr>
                </thead>
          ...
          {notas && notas.length > 0 && (
            <div className="border-l-2 border-deloitte-green pl-3 py-1 space-y-1">
              {notas.map((n, i) => (
                <div key={i} className="text-[11.5px] text-deloitte-slate leading-relaxed">• {n}</div>
              ))}
            </div>
          )}
        </div>
        ...
        {/* Footer con fuente */}
        <div className="px-6 py-3 border-t border-deloitte-line bg-deloitte-paper/40 flex items-center justify-between text-[11px]">
          <div className="text-deloitte-mute">
            <span className="font-semibold">Fuente:</span> {fuente}
          </div>
          <button
            onClick={onClose}
            className="px-4 py-1 bg-deloitte-ink text-white rounded text-[11px] font-semibold hover:bg-deloitte-slate"
          >
            Cerrar
          </button>
        </div>
